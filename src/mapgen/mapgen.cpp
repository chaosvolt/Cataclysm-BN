...
auto mapgen_constructor::generate(
    const tripoint_abs_omt& omt_pos, const time_point& when, const mapgen_generate_options& options)
    -> mapgen_result {
    ZoneScopedN("map_generate");
    ZoneValue(static_cast<uint64_t>(omt_pos.z() + OVERMAP_DEPTH));
    dbg(DL::Info) << "mapgen_constructor::generate( p[" << omt_pos << "], when[" << to_string(when)
                  << "] )";
    ...
    if (!ensure_omt_submaps(when)) { return {}; }
    ...
    mapgendata dat(omt_pos, *this, density, when, nullptr, omap);
    {
        ...
        if (draw_result.needs_main_thread()) { return draw_result; }
    }
    ...
    if (!commit_generated_submaps()) { return {}; }
...
public:
    ...
    auto get_mapgen_params(mapgen_parameter_scope scope, const std::string& context) const
        -> mapgen_parameters {
...
public:
     ...
     */
    auto has(const std::string& key) const -> bool { return mapgens_.contains(key); }
    ...
    auto generate(mapgendata& dat, const std::string& key, const int hardcoded_weight = 0) const
        -> bool {
    ...
    auto get_map_special_params(const std::string& key) const -> mapgen_parameters {
...
void load_mapgen(const JsonObject& jo) {
    ...
    if (jo.has_array("om_terrain")) {
        ...
        debugmsg(
            "mapgen entry requires \"om_terrain\" or \"nested_mapgen_id\"(string, array of strings, or array of array of strings)\n%s\n",
            jo.str());
    }
}
...
auto mapgen_function_json_base::calc_index(const point_rel_ms& p) const -> size_t {
...
auto mapgen_function_json_base::check_inbounds(
    const jmapgen_int& x, const jmapgen_int& y, const JsonObject& jso) const -> bool {
...
auto jmapgen_int::get() const -> int { return val == valmax ? val : rng(val, valmax); }
...
auto mapgen_function_json_base::get_args(const mapgendata& md, mapgen_parameter_scope scope) const
    -> mapgen_arguments {
...
map_key::map_key(const std::string& s): str(s) {
    if (utf8_width(str) != 1) { debugmsg("map key '%s' must be 1 column", str); }
}
...
template <typename T> static auto is_null_helper(const string_id<T>& id) -> bool {
    return id.is_null();
}

template <typename T> static auto is_null_helper(const int_id<T>& id) -> bool {
    return id.id().is_null();
}

static auto is_null_helper(const std::string&) -> bool { return false; }
...
template <typename T> struct make_null_helper<string_id<T>> {
    auto operator()() const -> string_id<T> { return string_id<T>::NULL_ID(); }
};

template <typename T> struct make_null_helper<int_id<T>> {
    auto operator()() const -> int_id<T> { return string_id<T>::NULL_ID().id(); }
};

template <typename T> static auto to_string_id_helper(const string_id<T>& id) -> string_id<T> {
    return id;
}

template <typename T> static auto to_string_id_helper(const int_id<T>& id) -> string_id<T> {
    return id.id();
}

static auto to_string_id_helper(const std::string& s) -> std::string { return s; }

template <typename T> static auto is_valid_helper(const string_id<T>& id) -> bool {
    return id.is_valid();
}

template <typename T> static auto is_valid_helper(const int_id<T>&) -> bool { return true; }

static auto is_valid_helper(const std::string&) -> bool { return true; }
...
public:
    ...
    struct value_source {
        ...
        virtual auto get(const mapgendata&) const -> Id = 0;
        ...
        virtual auto all_possible_results(const mapgen_parameters&) const
            -> std::vector<StringId> = 0;
    };

    struct null_source: value_source {
        auto get(const mapgendata&) const -> Id override { return make_null_helper<Id>{}(); }
        ...
        auto all_possible_results(const mapgen_parameters&) const
            -> std::vector<StringId> override {
    ...
    struct id_source: value_source {
        ...
        auto get(const mapgendata&) const -> Id override { return id; }
    ...
    struct param_source: value_source {
        ...
        auto get(const mapgendata& dat) const -> Id override {
            if (fallback) {
        ...
        auto all_possible_results(const mapgen_parameters& params) const
            -> std::vector<StringId> override {
    ...
    struct distribution_source: value_source {
        ...
        auto get(const mapgendata&) const -> Id override { return *list.pick(); }
    ...
    auto is_null() const -> bool { return is_null_; }
    ...
    auto get(const mapgendata& dat) const -> Id { return source_->get(dat); }
    auto all_possible_results(const mapgen_parameters& params) const -> std::vector<StringId> {
...
template <> auto enum_to_string<mapgen_parameter_scope>(mapgen_parameter_scope v) -> std::string {
    switch (v) {
        // *INDENT-OFF*
...
auto mapgen_parameter::type() const -> cata_variant_type { return type_; }

auto mapgen_parameter::get(const mapgendata& md) const -> cata_variant {
    return cata_variant::from_string(type_, default_->get(md));
}

auto mapgen_parameter::all_possible_values(const mapgen_parameters& params) const
...
auto mapgen_parameters::params_for_scope(mapgen_parameter_scope scope) const -> mapgen_parameters {
    mapgen_parameters result;
...
auto mapgen_parameters::get_args(const mapgendata& md, mapgen_parameter_scope scope) const
    -> mapgen_arguments {
...
public:
    ...
    auto phase() const -> mapgen_phase override {
    ...
    auto has_vehicle_collision(const mapgendata& dat, const point_rel_ms& p) const
        -> bool override {
...
public:
    ...
    auto phase() const -> mapgen_phase override { return underlying_piece->phase(); }
    void check(const std::string& context, const mapgen_parameters& params) const override {
...
public:
    ...
    auto phase() const -> mapgen_phase override { return mapgen_phase::faction_ownership; }
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        dat.m.place_items(group_id, chance.get(), point_omt_ms(x.val, y.val),
                          point_omt_ms(x.valmax, y.valmax), true, calendar::start_of_cataclysm);
    }
};
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        if (rng(0, 99) < chance) {
            ...
            dat.m.spawn_items(
                point_omt_ms(rng(x.val, x.valmax), rng(y.val, y.valmax)), std::move(spawn));
        }
    }
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        dat.m.place_spawns(
            chosen_id, chance.get(), point_omt_ms(x.val, y.val), point_omt_ms(x.valmax, y.valmax),
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        if (!chosen_group.is_null()) {
            ...
            dat.m.add_spawn(spawn_details.name, spawn_count * pack_size.get(), {x.get(), y.get()},
                            friendly, -1, mission_id, name);
        } else {
            mtype_id chosen_type = ids.pick()->get(dat);
            if (!chosen_type.is_null()) {
                dat.m.add_spawn(chosen_type, spawn_count * pack_size.get(), {x.get(), y.get()},
                                friendly, -1, mission_id, name);
            }
        }
    }
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        dat.m.add_vehicle(chosen_id, point_omt_ms(x.get(), y.get()), random_entry(rotation), fuel,
                          status, true, locked, has_keys, place_beyond_bounds);
    }
    auto has_vehicle_collision(const mapgendata& dat, const point_rel_ms& p) const
        -> bool override {
        return dat.m.veh_at(point_omt_ms(p.x(), p.y())).has_value();
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        const int spawn_count =
            ((c == 100) ? 1 : roll_remainder(c * spawn_rate / 100.0f)) * amount.get();

        const point_omt_ms p = {x.get(), y.get()};

        detached_ptr<item> itm = item::in_its_container(
            item::spawn(chosen_id, calendar::start_of_cataclysm));

        const int spawns = dat.m.edit_item_for_spawn_rate(*itm);
        const int total_spawns = spawn_count * spawns;
        if (total_spawns == 0) { return; }

        for (int i = 1; i < total_spawns; i++) {
            detached_ptr<item> tmp = item::spawn(*itm);
            if (activate_on_spawn) { tmp->activate(); }
...
public:
    ...
    jmapgen_furniture(const JsonObject& jsi): id(jsi.get_member("furn")) {
        ...
        if (jsi.has_array("colors")) { palette = MapgenColorPalette::define_new_palette(jsi); }
    }
    ...
    auto phase() const -> mapgen_phase override { return mapgen_phase::furniture; }
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        if (palette.is_valid()) {
            unsigned int rand_seed =
                (dat.pos.x() % 256) + (dat.pos.y() % 256) * 256 + (dat.pos.z() % 256) * 256 * 256;
...
public:
    ...
    auto is_nop() const -> bool override { return id.is_null(); }
    auto phase() const -> mapgen_phase override { return mapgen_phase::terrain; }
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        dat.m.make_rubble(
            point_omt_ms(x.get(), y.get()), chosen_rubble_type, chosen_floor_type, overwrite);
    }
...
public:
    ...
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
        ...
        dat.m.furn_set(r, f_null);
        computer* cpu = dat.m.add_computer(r, name.translated(), security);
...
public:
    ...
    auto phase() const -> mapgen_phase override { return mapgen_phase::transform; }
    void apply(
...
public:
    ...
    auto phase() const -> mapgen_phase override { return mapgen_phase::removal; }
    void apply(const mapgendata& dat, const jmapgen_int& x, const jmapgen_int& y) const override {
...
private:
    ...
    public:
        ...
        auto test(const mapgendata& dat) const -> bool {
...
public:
    ...
    jmapgen_nested(const JsonObject& jsi)
        ...
        if (jsi.has_member("rotation")) { rotation = jmapgen_int(jsi, "rotation"); }
    }

    auto get_entries(const mapgendata& dat) const -> const weighted_int_list<std::string>& {
    ...
    auto phase() const -> mapgen_phase override { return mapgen_phase::nested_mapgen; }
            ...
            if (entry.test_string()) {
                ...
                if (piece_and_count_jarr.size() != 2) {
                    piece_and_count_jarr.throw_error(
                        "Array must have exactly two entries: the object, then the count.");
                }
...
template <> auto string_id<mapgen_palette>::obj() const -> const mapgen_palette& {
...
template <> auto string_id<mapgen_palette>::is_valid() const -> bool {
...
auto mapgen_palette::load_temp(
    const JsonObject& jo, const std::string& src, const std::string& context) -> mapgen_palette {
...
void mapgen_palette::reset() { palettes.clear(); }
...
auto mapgen_palette::load_internal(
    ...
    // conflicts with the items entry with refers to item groups.
    ...
    new_pal
        .load_place_mapings<jmapgen_ter_furn_transform>(jo, "ter_furn_transforms", format_placings);
 ...
 */
auto jmapgen_setmap::apply(
    const mapgendata& dat, const point_rel_ms& offset,
    std::function<point_omt_ms(const point_omt_ms&)> func) const -> bool {
...
auto jmapgen_setmap::has_vehicle_collision(const mapgendata& dat, const point_rel_ms& offset) const
    -> bool {
...
auto mapgen_function_json_base::has_vehicle_collision(
    const mapgendata& dat, const point_rel_ms& offset) const -> bool {
...
auto mapgen_function_json::get_mapgen_params(mapgen_parameter_scope scope) const
    -> mapgen_parameters {
    return parameters.params_for_scope(scope);
}

void mapgen_function_json_nested::nest(
    const mapgendata& md, const point_rel_ms& offset, const int rotation) const {
    ...
    const auto apply_contents = [&](const mapgendata& active_md) -> void {
        {
            ...
            for (const auto& elem : setmap_points) { elem.apply(active_md, offset, rot_func); }
        }
...
auto jmapgen_objects::has_vehicle_collision(const mapgendata& dat, const point_rel_ms& offset) const
    -> bool {
    for (auto& obj : objects) {
        auto where = obj.first;
        where.offset(-offset);
...
/////////////
auto mapgen_constructor::draw_map(mapgendata& dat, const mapgen_generate_options& options)
    -> mapgen_result {
...
void mapgen_constructor::draw_office_tower(const mapgendata& dat) {
    ...
    const auto place_office_chairs = [&]() {
        ...
        for (int i = 0; i < num_chairs; i++) {
            add_vehicle(vproto_id("swivel_chair"), point_omt_ms(rng(6, 16), rng(6, 16)), 0_degrees,
                        -1, -1, false);
        }
    };
    ...
    if (terrain_type == "office_tower_1_entrance") {
        ...
        if (dat.monster_density() > 1) {
            place_spawns(GROUP_ZOMBIE, 2, point_omt_ms::zero(), point_omt_ms(12, 3),
                         dat.monster_density());
        } else {
        ...
        if ((dat.south() == "office_tower_1_entrance" && dat.east() == "office_tower_1")
            ...
            mapf::formatted_set_simple(
                ...
                ter_key, fur_key);
            if (dat.monster_density() > 1) {
                place_spawns(GROUP_ZOMBIE, 2, point_omt_ms::zero(), point_omt_ms(2, 8),
                             dat.monster_density());
            } else {
                place_spawns(GROUP_PLAIN, 1, point_omt_ms(5, 7), point_omt_ms(15, 20), 0.1);
            }
            place_items(item_group_id("office"), 75, point_omt_ms(4, 23), point_omt_ms(7, 23),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(4, 19), point_omt_ms(7, 19),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(4, 14), point_omt_ms(7, 14),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(5, 16), point_omt_ms(7, 16),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("fridge"), 80, point_omt_ms(14, 17), point_omt_ms(14, 17),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("cleaning"), 75, point_omt_ms(19, 17), point_omt_ms(20, 17),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(6, 12),
                        point_omt_ms(7, 12), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(12, 11),
                        point_omt_ms(12, 12), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(16, 11),
                        point_omt_ms(17, 12), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(4, 5), point_omt_ms(5, 5),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(11, 5),
                        point_omt_ms(12, 5), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(14, 5),
                        point_omt_ms(16, 5), false, calendar::start_of_cataclysm);
            place_office_chairs();
            ...
            mapf::formatted_set_simple(
                ...
                "ssssssssssssssssssssssss\n",
                ter_key, fur_key);
            place_items(item_group_id("office"), 75, point_omt_ms(19, 1), point_omt_ms(19, 3),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(17, 3), point_omt_ms(18, 3),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 90, point_omt_ms(8, 7), point_omt_ms(9, 7), false,
                        calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(19, 5),
                        point_omt_ms(19, 7), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cleaning"), 80, point_omt_ms(1, 7), point_omt_ms(2, 7),
                        false, calendar::start_of_cataclysm);
            if (dat.monster_density() > 1) {
            ...
            place_items(item_group_id("cleaning"), 75, point_omt_ms(3, 5), point_omt_ms(5, 5),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(10, 7), point_omt_ms(16, 8),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(15, 15),
                        point_omt_ms(19, 15), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(16, 12),
                        point_omt_ms(16, 13), false, calendar::start_of_cataclysm);
            place_items(item_group_id("cubical_office"), 75, point_omt_ms(17, 19),
                        point_omt_ms(19, 19), false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(17, 21), point_omt_ms(19, 21),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("office"), 75, point_omt_ms(16, 11), point_omt_ms(17, 12),
                        false, calendar::start_of_cataclysm);
            place_items(item_group_id("cleaning"), 75, point_omt_ms(8, 20), point_omt_ms(10, 20),
                        false, calendar::start_of_cataclysm);
            if (dat.monster_density() > 1) {
                place_spawns(GROUP_ZOMBIE, 2, point_omt_ms::zero(), point_omt_ms(9, 15),
                             dat.monster_density());
            } else {
        ...
        if (dat.monster_density() > 1) {
            ...
            place_spawns(GROUP_PLAIN, 1, point_omt_ms::zero(), point_omt_ms(EAST_EDGE, SOUTH_EDGE),
                         0.1);
        }
        ...
        if ((dat.south() == "office_tower_b_entrance" && dat.east() == "office_tower_b")
            ...
            mapf::formatted_set_simple(
                ...
                b_ter_key, b_fur_key);
            if (dat.monster_density() > 1) {
                place_spawns(GROUP_ZOMBIE, 2, point_omt_ms::zero(),
                             point_omt_ms(EAST_EDGE, SOUTH_EDGE), dat.monster_density());
            } else {
                place_spawns(GROUP_PLAIN, 1, point_omt_ms::zero(),
                             point_omt_ms(EAST_EDGE, SOUTH_EDGE), 0.1);
            }
            if (dat.west() == "office_tower_b_entrance") {
                ...
                rotate(3);
                if (x_in_y(1, 5)) { add_vehicle(vproto_id("car"), point_omt_ms(6, 4), 0_degrees); }
                if (x_in_y(1, 3)) {
            ...
            if (dat.north() == "office_tower_b_entrance") {
                rotate(1);
                if (x_in_y(1, 5)) { add_vehicle(vproto_id("car"), point_omt_ms(8, 15), 0_degrees); }
                if (x_in_y(1, 5)) {
            ...
            if (dat.west() == "office_tower_b" && dat.north() == "office_tower_b") {
                ...
                if (x_in_y(1, 3)) { add_vehicle(vproto_id("car"), point_omt_ms(4, 6), 90_degrees); }
            }
        }
    }
}
        ...
        if (dat.zlevel() == 0) {
            ...
            switch (1 + std::abs(abs_offset_.y() + dat.zlevel() + 4) % 4) { // TODO: More varieties!
                ...
                case 1:
                    ...
                    square(this, t_rock, point_omt_ms(SEEX + 2, 0),
                           point_omt_ms(EAST_EDGE, SOUTH_EDGE));
                ...
                case 2:
                    ...
                    square(this, t_water_dp, point_omt_ms(SEEX * 2 - 5, 4),
                           point_omt_ms(SEEX * 2 - 4, 6));
                    place_spawns(GROUP_SEWER, 1, point_omt_ms(1, SEEX * 2 - 5),
                                 point_omt_ms(1, SEEX * 2 - 5), 1, true);

                    square(this, t_water_dp, point_omt_ms(4, SEEY * 2 - 5),
                           point_omt_ms(6, SEEY * 2 - 4));

                    square(this, t_water_dp, point_omt_ms(SEEX * 2 - 5, SEEY * 2 - 5),
                           point_omt_ms(SEEX * 2 - 4, SEEY * 2 - 4));

                    square(this, t_rock, point_omt_ms(0, SEEY * 2 - 2),
                           point_omt_ms(SEEX - 1, SOUTH_EDGE));
                    square(this, t_rock, point_omt_ms(SEEX + 2, SEEY * 2 - 2),
                           point_omt_ms(EAST_EDGE, SOUTH_EDGE));
                    line(this, t_grate, point_omt_ms(SEEX, 1), point_omt_ms(SEEX + 1, 1)); // To
                                                                                           // drain
                                                                                           // the
                                                                                           // water
                ...
                case 4: { // Toggling walls maze
                    ...
                    square(this, t_rock, point_omt_ms(SEEX + 2, SEEY * 2 - 2),
                           point_omt_ms(EAST_EDGE, SOUTH_EDGE));
                    square(this, t_rock, point_omt_ms(SEEX + 5, 2),
                           point_omt_ms(EAST_EDGE, SEEY * 2 - 3));
            ...
            if (dat.above() == "temple_stairs") {
                line(this, t_stairs_up, point_omt_ms(SEEX, SOUTH_EDGE),
                     point_omt_ms(SEEX + 1, SOUTH_EDGE));
            }
...
void mapgen_constructor::draw_mine(mapgendata& dat) {
    ...
    if (terrain_type == "mine" || terrain_type == "mine_down") {
            ...
            case 3: {
                ...
                for (int i = wreck_location.x() - 3; i < wreck_location.x() + 3; i++) {
                    for (int j = wreck_location.y() - 3; j < wreck_location.y() + 3; j++) {
                        if (!one_in(4)) { make_rubble(point_omt_ms(i, j), f_wreckage); }
                    }
                }
            } break;
            ...
            if (open.empty()) { // We'll have to build it in the center
                ...
                do {
                    ...
                    for (int i = p.x(); (i <= p.x() + 5) && okay; i++) {
                        for (int j = p.y(); (j <= p.y() + 5) && okay; j++) {
                            if (ter(point_omt_ms(i, j)) != t_rock_floor) { okay = false; }
                        }
                    }
                    if (!okay) { tries++; }
                } while (!okay && tries < 10);
                ...
                switch (random_entry(open)) {
                    case direction::NORTH:
                        square(this, t_rock_floor, point_omt_ms(SEEX - 3, 6),
                               point_omt_ms(SEEX + 2, SEEY));
                        line(this, t_slope_down, point_omt_ms(SEEX - 2, 6),
                             point_omt_ms(SEEX + 1, 6));
                        break;
                    case direction::EAST:
                        square(this, t_rock_floor, point_omt_ms(SEEX + 1, SEEY - 3),
                               point_omt_ms(SEEX * 2 - 7, SEEY + 2));
                        line(this, t_slope_down, point_omt_ms(SEEX * 2 - 7, SEEY - 2),
                             point_omt_ms(SEEX * 2 - 7, SEEY + 1));
                        break;
                    case direction::SOUTH:
                        square(this, t_rock_floor, point_omt_ms(SEEX - 3, SEEY + 1),
                               point_omt_ms(SEEX + 2, SEEY * 2 - 7));
                        line(this, t_slope_down, point_omt_ms(SEEX - 2, SEEY * 2 - 7),
                             point_omt_ms(SEEX + 1, SEEY * 2 - 7));
                        break;
                    case direction::WEST:
                        square(this, t_rock_floor, point_omt_ms(6, SEEY - 3),
                               point_omt_ms(SEEX, SEEY + 2));
                        line(this, t_slope_down, point_omt_ms(6, SEEY - 2),
                             point_omt_ms(6, SEEY + 1));
                        break;
                    default:
                        break;
                }
        ...
        if (dat.above() == "mine_down") { // Don't forget to build a slope up!
            ...
            if (open.empty()) { // We'll have to build it in the center
                ...
                switch (random_entry(open)) {
                    case direction::NORTH:
                        line(this, t_slope_up, point_omt_ms(SEEX - 2, 6),
                             point_omt_ms(SEEX + 1, 6));
                        break;
                    case direction::EAST:
                        line(this, t_slope_up, point_omt_ms(SEEX * 2 - 7, SEEY - 2),
                             point_omt_ms(SEEX * 2 - 7, SEEY + 1));
                        break;
                    case direction::SOUTH:
                        line(this, t_slope_up, point_omt_ms(SEEX - 2, SEEY * 2 - 7),
                             point_omt_ms(SEEX + 1, SEEY * 2 - 7));
                        break;
                    case direction::WEST:
                        line(this, t_slope_up, point_omt_ms(6, SEEY - 2),
                             point_omt_ms(6, SEEY + 1));
                        break;
                    default:
                        break;
        ...
        if (dat.east() == "mine") {
            square(this, t_rock_floor, point_omt_ms(SEEX * 2 - 4, SEEY),
                   point_omt_ms(EAST_EDGE, SEEY + 1));
        }

        if (dat.south() == "mine") {
            square(this, t_rock_floor, point_omt_ms(SEEX, SEEY * 2 - 4),
                   point_omt_ms(SEEX + 1, SOUTH_EDGE));
        }
        ...
        place_spawns(GROUP_DOG_THING, 1, point_omt_ms(SEEX, SEEX), point_omt_ms(SEEX + 1, SEEX + 1),
                     1, true, true);
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            if (one_in(10) && ter(point_omt_ms(point_omt_ms(i, j))) == t_rock_floor) {
    ...
    if (is_ot_match("subway", terrain_type,
        ...
        if (is_ot_match("sewer", dat.north(), ot_match_type::type)
            ...
            if (connects_to(dat.north(), 2)) {
                for (int i = SEEX - 2; i < SEEX + 2; i++) {
                    for (int j = 0; j < SEEY; j++) { ter_set(point_omt_ms(i, j), t_sewage); }
                }
            } else {
                for (int j = 0; j < 3; j++) {
        ...
        if (is_ot_match("sewer", dat.south(), ot_match_type::type)
            ...
            if (connects_to(dat.south(), 0)) {
                for (int i = SEEX - 2; i < SEEX + 2; i++) {
                    for (int j = SEEY; j < SEEY * 2; j++) { ter_set(point_omt_ms(i, j), t_sewage); }
                }
            } else {
                ...
                for (int y = 0; y < 4; y++) {
                    for (int x = SEEX * 2 - 4; x < SEEX * 2; x++) {
                        if (x - y > SEEX * 2 - 4) {
                            ...
                            if (ter(point_omt_ms(x, y)) == t_grass
                                || ter(point_omt_ms(x, y)) == t_dirt
...
void map::place_spawns(
    ...
    const tripoint_bub_ms& p2, const float density, const bool individual, const bool friendly,
    const std::string& name, const int mission_id) {
...
auto map::place_npc(const tripoint_bub_ms& p, const string_id<npc_template>& type, bool force)
    -> character_id {
...
auto map::place_items(
    ...
    const int ammo) -> std::vector<item*> {
...
auto map::add_vehicle(
    const std::variant<vgroup_id, vproto_id>& type_, const tripoint_bub_ms& p,
 ...
 */
auto map::add_vehicle_to_map(std::unique_ptr<vehicle> veh, const bool merge_wrecks)
...
auto map::add_computer(const tripoint_bub_ms& p, const std::string& name, int security)
    -> computer* {
    return get_mapbuffer().add_computer(
...
void map::rotate(int turns, const bool setpos_safe) {
    ...
    for (const shared_ptr_fast<npc>& i : npcs) {
        ...
        if (proj.quotient != abs_omt) { continue; }
    ...
    for (int j = 0; j < 2; ++j) {
        for (int i = 0; i < 2; ++i) {
            ...
            for (auto& veh : sm->vehicles) { veh->abs_sm_pos = map_local_to_abs(*this, p); }
    ...
    switch (prop) {
        ...
        case ARTPROP_WARM:
            for (int i = cp.x() - 5; i <= cp.x() + 5; i++) {
                for (int j = cp.y() - 5; j <= cp.y() + 5; j++) {
                    if (furn(tripoint_bub_ms(i, j, player_pos.z())) == f_rubble) {
                        add_field({i, j, player_pos.z()}, fd_fire_vent,
                                  1 + (rl_dist(cp.xy(), point_bub_ms(i, j)) % 3));
                    }
                }
            }
            break;
...
void fill_background(mapgen_constructor* m, const ter_id& type) { m->draw_fill_background(type); }
void fill_background(mapgen_constructor* m, ter_id (*f)()) { m->draw_fill_background(f); }
void square(
    mapgen_constructor* m, const ter_id& type, const point_omt_ms& p1, const point_omt_ms& p2) {
...
void rough_circle_furn(mapgen_constructor* m, const furn_id& type, const point_omt_ms& p, int rad) {
...
void add_corpse(mapgen_constructor* m, const point_omt_ms& p) { m->add_corpse(p); }
...
auto update_mapgen_function_json::update_map(
    const mapgendata& md, const point_rel_ms& offset, const bool verify) const -> bool {
...
auto run_mapgen_update_func(
    const std::string& update_mapgen_id, mapgendata& dat, const bool cancel_on_collision) -> bool {
    const auto update_function = update_mapgen.find(update_mapgen_id);
    if (update_function == update_mapgen.end() || update_function->second.empty()) { return false; }
...
auto has_mapgen_for(const std::string& key) -> bool { return oter_mapgen.has(key); }

namespace mapgen {

auto has_update_id(const mapgen_id& id) -> bool { return update_mapgen.contains(id); }
