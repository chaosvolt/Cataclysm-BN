...
auto prepare_item_for_placement(const item_placement_options& options) -> bool {
    if (!options.item_to_place) { return false; }

    if (g == nullptr) { return true; }

    auto& here = g->m;
    if (here.get_bound_dimension() != options.dim_id) { return true; }

    const auto local = abs_to_map_local(here, options.p);
    if (!here.inbounds(local)) { return true; }

    if (options.item_to_place->is_food()) {
        options.item_to_place =
            item::process(std::move(options.item_to_place), nullptr, local, false, 1);
        if (!options.item_to_place) { return false; }
    }
    ...
    if (here.has_flag("DESTROY_ITEM", local)) { return false; }

    if (options.item_to_place->has_flag(flag_ACT_IN_FIRE)
        && here.get_field(local, fd_fire) != nullptr) {
        if (options.item_to_place->has_flag(flag_BOMB)
            && options.item_to_place->is_transformable()) {
            options.item_to_place->convert(
                dynamic_cast<const iuse_transform*>(
                    options.item_to_place->type->get_use("transform")->get_actor_ptr())
                    ->target);
        }
