...
1. MODのJSONおよびLuaソースファイルから文字列を抽出し、POTファイルを作成する
2. このPOTファイルから、対象言語のPOファイルを作成する
3. POファイルを翻訳済み文字列で埋める
4. POファイルをMOファイルにコンパイルする
5. MOファイルをMODファイル内に配置する

MODは時間の経過とともに変更され、それに伴い文字列も変更される可能性があります。既存の翻訳を更新する手順は以下のとおりです。

1. MODの JSON およびLuaソースファイルから文字列を抽出し、新しいPOTファイルを作成する
